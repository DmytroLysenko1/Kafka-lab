...
[run 1](results/run-2026-09-22-015105.log) · [run 2](results/run-2026-09-22-015633.log) ·
[run 3](results/run-2026-09-22-020919.log) · [Java client defaults](results/java-client-defaults.log)

| Linger | Records per batch | Median | p99 | zstd | snappy |
|---|---|---|---|---|---|
| 0 | 4.2–8.5 | 1.9–3.3 ms | 10–34 ms, unstable | 2.9–3.7× | 2.05–2.29× |
| 5 ms — Java's default value, set on franz-go | 21.3–24.4 | 6.5–7.3 ms | 12.5–18.7 ms | 4.75–4.81× | 2.82–2.84× |
| 10 ms — franz-go's default | 33.7–37.1 | 8.5–10.2 ms | 17.2–23.4 ms | 5.14–5.17× | 2.95–2.97× |
| 50 ms, 16 KiB | 39.4–40.2 | 11.7–12.2 ms | 23.8–28.0 ms | 5.23–5.24× | 2.99× |
| 50 ms, default | 137.7–152.5 | 30.8–36.6 ms | 58.2–68.0 ms | 5.64–5.65× | 3.28× |
...
the grid is `5ms / 16KiB / none`. Ranges are over all three runs, every codec and both batch
...
itself. On franz-go, 10 ms costs about 2 ms of median against 5 ms and buys batches half as
large again.

**No cell fell behind** — `drain`, the time from the last record offered to the last
acknowledgement, was 1.3–12.2 ms and `failed` 0 in all 96 cells. An earlier version of this
sweep printed `acked/s` instead, which divided by the same window as `offered/s` and so
could never have shown a backlog.

**What the instrument got wrong.** The byte columns come from franz-go's batch-written
hook, which it calls from a goroutine nothing joins — not `Flush`, not `Close`. Reading
them right after closing the client could miss the cell's last batches, and at linger 0,
where batches are tiny and numerous, that is not a rounding error: the same grid measured
4.2–8.5 records per batch once the sweep waits for the hook, against ≈5 before, and zstd
2.9–3.7× against 2.87–3.22×. Every run published here waits for the hook to account for
every acknowledged record, and fails the cell if it does not.

**Not shown:** throughput and codec CPU cost, since at this load neither differs; both
need a saturated run — [exp-17b](../exp-17b-saturation/). Nor is the load exactly fixed:
the 5 ms ticker drops ticks, so offered/s ran 19 479–20 000 against 20 000 asked, within
about 2.6%.
