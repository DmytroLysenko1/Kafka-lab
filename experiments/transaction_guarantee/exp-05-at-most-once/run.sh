#!/usr/bin/env bash
# exp-05 — at-most-once, the offset committed before the write.
#
...
binary="$(mktemp -d)/exp-05"
trap 'rm -rf "$(dirname "$binary")"' EXIT
go build -o "$binary" ./experiments/transaction_guarantee/exp-05-at-most-once
run() { "$binary" -run-id "$run_id" -payments "$payments" "$@"; }

{
  echo "exp-05 — at-most-once, the offset committed before the write"
  echo "date: $stamp"
