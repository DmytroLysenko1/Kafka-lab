...
| exp-13 | broker killed under load: outbox backlog grows, API keeps answering, relay drains afterwards | **measured**: at 10 payments/s, not one was refused in either cell. One broker down cost a leader election and 88–133 records of backlog, drained before the broker was back. Two brokers down — below `min.insync.replicas` — blocked publishing for the whole 30 s, backed up 303–309 records during the outage and 468–523 at the peak just after it, and the relay worked it off in 18.3–24.2 s over three runs. The relay's own backlog gauge read 1 throughout — it is set at the end of a sweep, and none ended. The cluster meanwhile reported 0 under-replicated and 0 leaderless partitions: with two of three controllers dead there is no quorum to update the metadata, so the survivor keeps serving the picture from before the kill ([exp-13](../../experiments/transaction_guarantee/exp-13-broker-outage/)) |
