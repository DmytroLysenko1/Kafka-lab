...
| [exp-08](exp-08-acks/) | `acks=1` with the followers paused under a leader kill, against `acks=all` with `min.insync.replicas=3` | **`acks=all` refused all 2 000 with `NOT_ENOUGH_REPLICAS`; `acks=1` with the followers paused lost all 2 000 it had acknowledged; `acks=all` under that same pause acknowledged 0 and lost 0** |
| [exp-09](exp-09-reordering/) | idempotence off, five requests in flight, repeated refusals | **0 reordered in five runs; 291, 80, 80, 80 and 191 duplicated, and in each of the three runs that counted retries the duplicates were exactly the records appended before a `REQUEST_TIMED_OUT` — a timeout duplicate, not a franz-go one; idempotent: zero of both** |
...
| [exp-10d](exp-10d-hanging-transaction/) | a transaction left open | **unrelated records invisible to `read_committed` for 23.2–27.3 s behind a 20 s timeout** |
| [exp-14](exp-14-rebalance-strategies/) | a second consumer joins: eager, cooperative-sticky and KIP-848 | **eager revoked all six partitions, and no gap outgrew the baseline poll cycle; cooperative stopped only the three that moved, for 0.51–0.68 s; KIP-848 for 4.9–6.5 s, around its heartbeat; nothing lost or handled twice** |
...
| [exp-13](exp-13-broker-outage/) | brokers killed under the running service at 10 payments/s | **not one payment refused; one broker down cost 88–133 records of backlog, two cost 303–309 during the outage and 468–523 at the peak after it, with an 18–24 s catch-up — and the cluster reported 0 under-replicated throughout, having lost the quorum that would update it** |
...
| [exp-17](exp-17-batching-sweep/) | linger × batch size × codec under fixed load | **linger decides batching, and batching roughly doubles compression: zstd 2.9× → 5.65× on the same bytes, zstd ≈1.7× snappy at equal batching; Java's default is 5 ms, not 0** |
