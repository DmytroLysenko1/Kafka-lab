...
sequenceDiagram
    ...
    rect rgba(245, 158, 11, 0.18)
        ...
        alt the batch is handled inside that budget
            ...
            CO-->>C: 15. UNKNOWN_MEMBER_ID, p3 already belongs to another member
...
is the only step here that can lose the partition mid-batch — exp-16 measured a member
removed while it held a batch (exp-02, exp-16).*
...
| 13 | Deserialization, business logic and database writes — the entire time budget lives in this one pause. | Take too long and the group moves on without you. Heartbeats keep flowing from a background thread, so the session timeout is not what fires: it is the rebalance timeout. In the Java client the trigger is `max.poll.interval.ms`, which doubles as the rebalance timeout. **franz-go has no poll watchdog at all** — it exposes `kgo.RebalanceTimeout`, `kgo.SessionTimeout` and `kgo.BlockRebalanceOnPoll` instead, so a slow handler goes unnoticed until a rebalance actually happens. Without `BlockRebalanceOnPoll` the rebalance does not wait for it at all; with it, the handler holds the rebalance, and after `RebalanceTimeout` (60 s by default, against Java's 300 s) the member is removed without being told — exp-16 measured that at an 8 s timeout, and the removed member's next commit rewinding a partition 1 726–1 732 records. |
...
| 15–16 | The other exit from the pause, and the reason the frame is drawn at all. The member is not fenced for being slow as such: it is fenced because a rebalance started while it was busy and it did not rejoin within the rebalance timeout. Its commit then fails with `UNKNOWN_MEMBER_ID` — the coordinator has dropped it, so it is not a stale generation of a member the coordinator still knows but a member the coordinator no longer has — and the batch is handed to whoever owns the partition now (exp-16 logged exactly this). | Reading this as "a rebalance costs some latency". It costs a **reprocessed batch**: every row already written by the doomed member is written again by the new owner. A slow handler is therefore a duplicate generator, not only a lag generator — which is the whole argument for the inbox in [05](05-delivery-semantics.md). |

## The batch is the unit, not the record

A fetch returns whole batches as they lie on disk. `max.poll.records` slices what the
Java client hands to application code, but the bytes already crossed the network — and in
franz-go the same limit is an argument, `PollRecords(ctx, n)`, rather than a setting, which
is easy to miss when porting a config. Sizing a fetch for throughput can therefore deliver far
...
|---|---|---|
| exp-02 | one merchant sending four events in five, then more consumers | **85% of the topic landed on one partition, and adding members bought nothing measurable: 5.63 s with one, then 5.12, 4.95, 4.81 and 4.76 s at two, three, six and seven. The seventh member handled nothing at all, yet its cell is 0.05 s faster than the sixth — so the spread across five single-shot cells is run-to-run variance, not parallelism. The hot partition is served by exactly one member throughout, which is the ceiling.** [run](../../experiments/kafka_internals/exp-02-hot-partition/results/run-2026-09-20-200615.log) |
| exp-16 | a dependency outage and a lag spike, with a handler that retries inline against one that pauses fetching ([07](07-retry-dlq.md)) | **the same lag either way, 7 940–8 501 records drained in 10–13 s; inline: member removed, a commit refused, a stale commit rewinding a partition 1 726–1 732 records; paused: none of it** ([exp-16](../../experiments/transaction_guarantee/exp-16-lag-backpressure/)) |
