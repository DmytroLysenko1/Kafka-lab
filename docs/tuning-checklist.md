...
| `enable.idempotence` (`kgo.DisableIdempotentWrite` to turn off) | on in both | Leave it on. It removes duplicates from producer-side retries: in exp-09 the idempotent producer retried 100–251 records per run that the leader had already appended before answering `REQUEST_TIMED_OUT`, and the broker dropped every retry by its sequence number — zero duplicated, in five runs of 144 000–267 000 events. Its effect on ordering is not isolated here: the runs without it did not reorder either. It holds only within one producer session — a process restart needs a `transactional.id` to keep the guarantee, which no run here measures yet |
...
| partitioner | the **same** algorithm on both sides, with different thresholds: KIP-794's uniform sticky partitioner, in the Java client since 3.3 and in franz-go as `UniformBytesPartitioner(64 KiB, adaptive, keys)`. Java re-picks after `batch.size` (16 KiB by default), franz-go after 64 KiB (`kgo/partitioner.go`: "a redux of the StickyPartitioner, proposed in KIP-794") | Leave it. Note the shape for experiments: with a null key franz-go stays on one partition until **64 KiB** have been written, so a small test run shows no disorder at all and invites a false conclusion (exp-01) |
...
| `max.in.flight.requests.per.connection` (`kgo.MaxProduceRequestsInflightPerBroker`) | **Java 5, franz-go 1** | The clients differ here. Raising franz-go to 5 buys throughput on a high-latency link; it is safe *only* with idempotence on, which keeps sequence numbers in order. **What it costs without idempotence is client-specific.** In Java the textbook failure is reordering — a retried batch lands after a later one. franz-go drops to one request in flight on a partition after an error until a request succeeds, which is the likely reason exp-09 saw zero reordered in five runs with five in flight. What it did see was duplication — 291, 80, 80, 80 and 191 — and the three runs that counted the cause agree: the duplicates were exactly the records the leader appended and then answered `REQUEST_TIMED_OUT` for, which any client without idempotence resends. So the duplicates are not a franz-go trait; the absence of reordering probably is |
...
| `batch.size`, whose nearest franz-go setting is `kgo.ProducerBatchMaxBytes` — though that one "mirrors Kafka's `max.message.bytes`" in its own docs, and franz-go has no separate soft 16 KiB batching cap | **Java 16 KB, franz-go ≈1 MB (1 000 012 bytes)** | Raise with linger for bulk work; on its own it does nothing unless batches are actually filling. **It also caps linger**: at 50 ms with a 16 KiB ceiling the median was 11.7–12.2 ms, while the default ≈1 MB ceiling waited the linger out (30.8–36.6 ms median, 138–153 records). The 16 KiB batches averaged ~40 records, about 10 KB: franz-go sends every partition bound for a broker once any one is ready, so the first to fill takes the others with it half-full. At 0–10 ms the two sizes measured the same, because no batch came near 16 KiB. Cost: memory per partition in flight (exp-17) |
| `compression.type` (`kgo.ProducerBatchCompression`) | **Java none, franz-go snappy then none** | **Batching sets how much any codec can do; the codec still matters.** On the same payment payloads exp-17 measured zstd at 2.9–3.7× with linger 0, 4.75–4.81× at 5 ms, 5.14–5.17× at 10 ms and 5.64–5.65× at 50 ms; snappy, franz-go's default, at 2.05–2.29×, 2.82–2.84×, 2.95–2.97× and 3.28×. Small batches waste most of what any codec can do, and at equal batching zstd still compresses about 1.7× better than snappy. At 5 MB/s the codec made no measurable difference to latency, so zstd — 44–86 bytes per 250-byte record — cost nothing there. Flat out (exp-17b) the codecs finally differ: without compression the producer stalled at 45–89 MB/s on the wire and 180 000–356 000 records/s, while zstd delivered 2.1–4.7× the records for 34–67% more producer CPU per megabyte, snappy 1.7–4.6× and lz4 1.1–3.1× for no measurable extra CPU — fewer bytes to send saves about as much as compressing them costs. The spread across four runs is wide because the brokers share the host; the ordering held in every run. Bytes being the ceiling on this stand is the likely reading — the brokers share the host and write every byte three times — but it was not isolated: no broker, disk or network figure was taken. Where CPU is the ceiling instead, the order can reverse. Cost is producer CPU and, if the topic sets its own codec, broker recompression |
...
| `fetch.max.bytes` (`kgo.FetchMaxBytes`) | 50 MiB in both | The size of one poll's worth of work, in bytes. The record count is bounded separately, by the argument to `PollRecords(ctx, n)`: franz-go has no `max.poll.records` *setting*, but it has the lever, as a poll argument rather than a config option. The two are complementary — this one bounds what crosses the network, `PollRecords` bounds what reaches the handler |
...
| `FetchMaxBytes` / `FetchMaxPartitionBytes`, and `PollRecords(ctx, n)` | the first two size one poll's worth of work in bytes; the third bounds it in records, which is where `max.poll.records` went | the handler cannot finish a full poll inside the rebalance timeout |
...
| `max.poll.records` | 500 | not a *setting*: the same limit is the argument to `PollRecords(ctx, n)` |
...
| default partitioner for null keys | KIP-794 uniform sticky, re-picks after `batch.size` (16 KiB) | the same KIP-794 partitioner, re-picking after **64 KiB** |
