#!/usr/bin/env bash
# exp-05b — at-most-once through GreedyAutoCommit: the timer commits what was polled, handled or not.
#
...
binary="$(mktemp -d)/exp-05b"
trap 'rm -rf "$(dirname "$binary")"' EXIT
go build -o "$binary" ./experiments/transaction_guarantee/exp-05b-autocommit-greedy
run() { "$binary" -run-id "$run_id" -payments "$payments" "$@"; }

{
  echo "exp-05b — at-most-once through GreedyAutoCommit: the timer commits what was polled, handled or not"
  echo "date: $stamp"
