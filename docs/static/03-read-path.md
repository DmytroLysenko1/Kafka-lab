...
|---|---|---|
| exp-02 | one merchant sending four events in five, then more consumers | **85% of the topic landed on one partition. Draining it took 5.63 s with one consumer and 4.76 s with seven — 15% faster for seven times the members, because the hot partition is handled by exactly one of them. The seventh handled nothing.** [run](../../experiments/kafka_internals/exp-02-hot-partition/results/run-2026-09-20-200615.log) |
