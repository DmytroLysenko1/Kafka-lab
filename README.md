...
`outbox_backlog_records` (meant to grow while the broker is away — but set when a sweep ends, and during a full outage none ends: exp-13 saw it flat at 1 with 308 records waiting, so until that is fixed read the backlog from Postgres),
...
failure, because it is built on the stale backlog gauge. `make verify` checks
