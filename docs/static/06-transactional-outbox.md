...
|---|---|---|
| exp-10 | Kafka EOS covers Kafka only; the same run with a Postgres write needs outbox plus inbox | **measured (exp-10b)**: a crash mid-transaction left the Kafka output at exactly 1 000 and the Postgres rows at 1 050 — the abort left its records in the log but marked aborted, so no `read_committed` reader is given them, and committed no offsets — and it did nothing to the database [run](../../experiments/transaction_guarantee/exp-10b-database-boundary/results/run-2026-09-22-022201.log) |
| exp-13 | broker killed under load: outbox backlog grows, API keeps answering, relay drains afterwards | **measured**: at 10 payments/s, not one was refused in either cell. One broker down cost a leader election and 88–133 records of backlog, drained before the broker was back. Two brokers down — below `min.insync.replicas` — blocked publishing for the whole 30 s, backed up 303–309 records during the outage and 468–523 at the peak just after it, and the relay worked it off in 18.3–24.2 s over three runs. The relay's own backlog gauge read 1 throughout — it was set at the end of a sweep, and none ended; counted on its own tick since, it climbed to 460 through the same outage. The cluster meanwhile reported 0 under-replicated and 0 leaderless partitions: with two of three controllers dead there is no quorum to update the metadata, so the survivor keeps serving the picture from before the kill ([exp-13](../../experiments/transaction_guarantee/exp-13-broker-outage/)) |
