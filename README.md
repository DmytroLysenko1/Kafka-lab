...
idempotent insert and the outbox — and the outbox relay that publishes those events to Kafka
as Protobuf through the schema registry. **No HTTP port is open yet** and nothing consumes
the topic; the diagram is labelled accordingly rather than quietly implying otherwise.
